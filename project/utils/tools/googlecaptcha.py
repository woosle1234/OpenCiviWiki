...
"""
Google ReCAPTCHA API to present a CAPTCHA to people when needed.
"""

#ReCAPTCHA from Google to comply with anti-botting measures
...
def recaptcha_test(request):
    ...
    if r.status_code == 200:
        ...
        if not response_json.get('success'):
            raise ValidationError(ugettext("Please try again."))
    else:
        raise ValidationError(ugettext("Failed to contact reCAPTCHA API."))
...
def qacaptcha_test(request):
    ...
    for predefined_answer in settings.qa_answers.lower().splitlines():
        ...
        raise ValidationError(ugettext("Entered answer is incorrect."))
